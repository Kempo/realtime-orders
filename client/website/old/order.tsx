  ...
  return (
    <div className={styles.container}>
      <div className={styles.order}>
        ...
        {status.loading ? (
          <Loading />
        ) : status.lineItems.length === 0 ? (
          <Error />
        ) : (
          <Success order={status.lineItems} />
        )}
      </div>
    </div>
  );
...
function Error() {
  return (
    <React.Fragment>
      <h1>Whoops!</h1>
      <p>
        Something strange happened. <br /> Text{" "}
        {process.env.NEXT_PUBLIC_PHONE_NUMBER} for help or swing by Cedars of
        Lebanon!
      </p>
    </React.Fragment>
  );
...
function Loading() {
  return (
    <React.Fragment>
      <h1>Loading...</h1>
      <p>
        This might take a little longer than usual. Thank you for being patient!
      </p>
    </React.Fragment>
  );
}
...
function Success({ order }: { order: MenuItemType[] }) {
  ...
  return (
    <React.Fragment>
      ...
      <hr />
      <p className={styles.restaurantNote}>
        We <b>sincerely</b> appreciate you taking the time to order from us.{" "}
        <br /> Instructions for picking up your order can be found below.
      </p>
      <div className={styles.receipt}>
        ...
        <table>
          ...
          <tbody>
            {order.length > 0 &&
              order.map((el: MenuItemType, i: number) => (
                <tr key={i}>
                  ...
                  <td className={styles.amountTotal}>
                    {formatter.format(el.amountTotal / 100)}
                  </td>
                </tr>
              ))}
          </tbody>
          <tfoot>
            <tr>
              ...
              <td className={styles.amountTotal}>
                <b>{formatter.format(totalPrice / 100)}</b>
              </td>
            </tr>
          </tfoot>
        </table>
...
function Instructions() {
  return (
    <div className={styles.instructions}>
      <h1>Instructions:</h1>
      <p>
        Orders are approximately finished in <b>15</b> minutes. <br /> <br />{" "}
        Please pick up your order at Cedars of Lebanon.
      </p>
      <div className={styles.contactRow}>
        <Image
          src="/icons/location-sharp.svg"
          alt="Location"
          width="18"
          height="18"
        />
      ...
      <p>
        <b>When you arrive, tell the cashier your order and enjoy!</b>
      </p>
    </div>
  );
}
